...
#### 書き出し後の全item IDキー存在チェック（必須）

HTMLの全item ID（`r6: null` 以外）について、対応する `_cnt` キーがJSON内に存在するか確認する。

**チェック手順：**
1. `index.html` のFeeTable定義から `r6: null` でない全item IDを列挙する
2. 各item IDについて、JSON内に `t_{id}_cnt` または `k_{id}_cnt` が存在するか確認する
3. 存在しない場合はPDFで該当行を探す
   - PDFに0件と記載あり → 0を保存する
   - PDFに記載なし → nullのまま（JSONに入れない）
4. 全item IDのチェック完了後、次のステップへ進む

**このチェックをしないと：** 合計値（①〜⑧）が一致していても、個別の0件行が抜けてHTML上で赤表示になる。
...
-エラーや問題は全てユーザーへ報告し、判断を仰ぐ。勝手に先へ進めない
-バリデーションの結果を報告
