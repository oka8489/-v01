---
name: jisseki
description: レセコンPDF（調剤報酬統計表・加算種別内訳表）を読み取り、data/r7_extracted.json に保存し、ブラウザへ読み込んでバリデーションする。/jisseki で実行。
---
...
```
PDF → Claudeが全項目を読み取り → data/r7_extracted.json に書き出し → ブラウザで「実績読込」ボタンを押して読み込み → バリデーション → ユーザー確認
```
...
1. ブラウザでindex.htmlを開く（プレビューサーバー経由）
2. 「報酬改定」タブ →「R6 R7実績」サブタブを開く
3. 「実績読込」ボタンを押してJSONを読み込む
4. 各セクションの値がブラウザに表示されることを確認する
5. PDF（加算種別内訳表）を見て、"算定なし"となっているプルダウンで適切な項目を選択する

### 5. バリデーション

ブラウザに表示された値をPDF記載値と照合する。
...
| ⑩ | 赤表示チェック | ブラウザで赤枠・赤文字の項目がないか確認。赤があればPDFを再確認し、データ不足か0件かを判断。 |
