...
export const BASIC_FEES = [
  ...
  {
    id: 'renkei', label: '連携強化加算', category: 'basic', inputType: 'select', isSub: true,
    changeType: 'same',
    r6: { options: [{ value: 0, label: '算定なし' }, { value: 5, label: '加算（5点）' }] },
  },
  {
    id: 'dx8', label: '医療DX推進体制整備加算（8点）', category: 'basic', inputType: 'select', isSub: true,
  ...
  {
    id: 'dx6', label: '医療DX推進体制整備加算（6点）', category: 'basic', inputType: 'select', isSub: true,
  ...
  {
    id: 'dx10', label: '医療DX推進体制整備加算（10点）', category: 'basic', inputType: 'select', isSub: true,
    changeType: 'abolished_merged', changeNote: 'R8で電子的調剤情報連携体制整備加算（8点）に統合され廃止。',
    changePurpose: 'DX加算体系を簡素化し、電子処方箋・調剤情報連携の実装段階に一本化。',
  ...
  {
    id: 'baseup', label: '調剤ベースアップ評価料', category: 'basic', inputType: 'select',
  ...
  {
    id: 'bio', label: 'バイオ後続品調剤体制加算', category: 'basic', inputType: 'select', isSub: true,
...
export const MANAGEMENT_FEES = [
  ...
  },
  {
    ...
    changeType: 'abolished_merged', changeNote: 'R8で廃止。包括的評価から実績重視の評価へ転換。服薬管理指導料1イ/2イ（かかりつけ薬剤師区分）に統合した上で、フォローアップ加算（50点）・訪問加算（230点）を新設。R6は76点。',
  ...
  {
    id: 'tokutei_2', label: '特定薬剤管理指導加算2（抗悪性腫瘍）', category: 'management', inputType: 'select', isSub: true,
    changeType: 'same',
  ...
  {
    id: 'shoni', label: '小児特定加算', category: 'management', inputType: 'select', isSub: true,
    changeType: 'same',
  ...
  {
    id: 'kakaritsuke_fu', label: 'かかりつけ薬剤師フォローアップ加算', category: 'management', inputType: 'select',
...
export const HOMECARE_FEES = [
  ...
  {
    id: 'zaitaku_shoni', label: '小児特定加算（在宅）', category: 'homecare', inputType: 'select', isSub: true,
    changeType: 'same',
    r6: { options: [{ value: 0, label: '算定なし' }, { value: 350, label: '加算（350点）' }] },
  },
  {
  ...
  {
    id: 'yakan_homon', label: '夜間訪問加算', category: 'homecare', inputType: 'fixed', isSub: true,
    changeType: 'same',
    r6: { fixedPoints: 400 },
  },
  {
    id: 'kyujitsu_homon', label: '休日訪問加算', category: 'homecare', inputType: 'fixed', isSub: true,
    changeType: 'same',
    r6: { fixedPoints: 600 },
  },
  {
    id: 'shinya_homon', label: '深夜訪問加算', category: 'homecare', inputType: 'fixed', isSub: true,
    changeType: 'same',
    r6: { fixedPoints: 1000 },
  },
  {
  ...
  // R8 新設（在宅）
  {
    id: 'kakaritsuke_houmon', label: 'かかりつけ薬剤師訪問加算', category: 'homecare', inputType: 'select',
  ...
  {
    id: 'fukusu_houmon', label: '複数名薬剤管理指導訪問料', category: 'homecare', inputType: 'select',
...
export const LONGTERM_FEES = [
  ...
  {
    id: 'kaigo_yobo_kyotaku_1', label: '介護予防居宅療養管理指導費Ⅱ（1人・1棟）', category: 'longterm', inputType: 'fixed',
    unit: '単位', changeType: 'same',
    r6: { fixedPoints: 518 },
  },
  {
