...
**全ての応答・提案・回答は、必ず `docu/` ディレクトリ内の資料・文書を根拠とすること。**
...
「タスク追加」「タスク一覧」「〇〇を進行中にして」などのトリガーワードで発動。
data/tasks.jsonを直接操作してタスクのCRUD・ステータス管理を行う。
- `skill/tasks/SKILL.md` — タスク管理スキル
- `data/tasks.json` — タスクデータ（定義+ステータス）
